...
export async function getStaffRequests(_req: Request, res: Response) {
  try {
...
export async function createStaffRequest(req: Request, res: Response) {
  try {
    ...
    const newRequest = await prisma.staffRequest.create({ data: { userId: user.id, courseId, message } });
...
export async function updateStaffRequest(req: Request, res: Response) {
  try {
    ...
    const updatedRequest = await prisma.staffRequest.update({ where: { id }, data: { message } });
...
export async function answerStaffRequest(req: Request, res: Response) {
  try {
    const { id } = req.params as { id: string };
    const { status } = req.body;
    if (![Status.APPROVED, Status.REJECTED].includes(status.toUpperCase()))
      return res.status(400).json({ status: "fail", message: "Invalid status" });
    const request = await prisma.staffRequest.findUnique({ where: { id }, include: { user: true, course: true } });
    if (!request) return res.status(404).json({ status: "fail", message: "Request not found" });
    if (request.status !== Status.PENDING)
      return res.status(400).json({ status: "fail", message: "Request already answered" });
    await prisma.$transaction(async (tx) => {
      await tx.staffRequest.update({ where: { id }, data: { status } });
