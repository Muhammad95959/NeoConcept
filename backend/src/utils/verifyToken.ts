...
export async function verifyToken(token?: string) {
  ...
  if (!process.env.JWT_SECRET) throw new CustomError(ErrorMessages.AGORA_APP_ID_OR_CERTIFICATE_MISSING, 500, HTTPStatusText.ERROR);
  ...
  if (user.passwordChangedAt) {
    ...
    if (isPasswordChanged)
      throw new CustomError(ErrorMessages.PASSWORD_RECENTLY_CHANGED, 401, HTTPStatusText.FAIL);
  }
